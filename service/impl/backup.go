...
func (b *backupServiceImpl) ExportData(ctx context.Context) (*dto.BackupDTO, error) {
	...
	err = fillData(data, "comment", dal.Use(dal.GetDBByCtx(ctx)).Comment.WithContext(ctx).Find, err)
	err = fillData(data, "comment_black", dal.Use(dal.GetDBByCtx(ctx)).CommentBlack.WithContext(ctx).Find, err)
	err = fillData(data, "journal", dal.Use(dal.GetDBByCtx(ctx)).Journal.WithContext(ctx).Find, err)
	err = fillData(data, "link", dal.Use(dal.GetDBByCtx(ctx)).Link.WithContext(ctx).Find, err)
	err = fillData(data, "log", dal.Use(dal.GetDBByCtx(ctx)).Log.WithContext(ctx).Find, err)
	err = fillData(data, "menu", dal.Use(dal.GetDBByCtx(ctx)).Menu.WithContext(ctx).Find, err)
	err = fillData(data, "meta", dal.Use(dal.GetDBByCtx(ctx)).Meta.WithContext(ctx).Find, err)
	err = fillData(data, "option", dal.Use(dal.GetDBByCtx(ctx)).Option.WithContext(ctx).Find, err)
	err = fillData(data, "photo", dal.Use(dal.GetDBByCtx(ctx)).Photo.WithContext(ctx).Find, err)
	err = fillData(data, "post", dal.Use(dal.GetDBByCtx(ctx)).Post.WithContext(ctx).Find, err)
	err = fillData(data, "post_category", dal.Use(dal.GetDBByCtx(ctx)).PostCategory.WithContext(ctx).Find, err)
	err = fillData(data, "post_tag", dal.Use(dal.GetDBByCtx(ctx)).PostTag.WithContext(ctx).Find, err)
	err = fillData(data, "theme_setting", dal.Use(dal.GetDBByCtx(ctx)).ThemeSetting.WithContext(ctx).Find, err)
	err = fillData(data, "user", dal.Use(dal.GetDBByCtx(ctx)).User.WithContext(ctx).Find, err)
	if err != nil {
